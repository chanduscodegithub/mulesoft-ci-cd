...
IF EXIST "%BASE_DIR%\bin\agent-setup-2.6.0-amc-final.jar" move "%BASE_DIR%\bin\agent-setup-2.6.0-amc-final.jar" "%BASE_DIR%\tools"

for /f tokens^=2-5^ delims^=.-_+^" %%j in ('java -fullversion 2^>^&1') do set "jver=%%j%%k%%l%%m"
if %jver% GTR 1100000 goto j11

:nofips
java -jar "%BASE_DIR%\tools\agent-setup-2.6.0-amc-final.jar" %*
goto end
...
java -cp "%BC%;%BASE_DIR%\tools\agent-setup-2.6.0-amc-final.jar" com.mulesoft.agent.installer.AgentInstaller %*
